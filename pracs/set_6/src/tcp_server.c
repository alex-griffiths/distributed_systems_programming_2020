...
int main() 
{
        ...
        struct sockaddr_in6 server, client; /* address structures for server and
                                              clients */
        ...
        rs_sd = socket(PF_INET6, SOCK_STREAM, 0);
        if (rs_sd < 0) 
        {
         ...
         * Use in6addr_any so that it can recieve messages sent to any of
         * its interfaces.
        ...
        server.sin6_port = htons(TCP_PORT);
        ...
        while(1)
        {
                ...
                es_sd = accept(rs_sd, (struct sockaddr *) &client, 
                ...
                char client_buf[200];
                client_details = getnameinfo((struct sockaddr *) &client, 
                                             client_len, client_buf, 
                                             sizeof(client_buf), NULL, 0, 0);
                if (client_details != 0)
                {
